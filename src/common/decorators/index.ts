...
export const StringRequired = (name: string, min?: number, max?: number) =>
    applyDecorators(
        ...
        ...(min !== undefined
            ? [
                  MinLength(min, {
                      message: `${name} phải có ít nhất ${min} ký tự`
                  })
              ]
            : []),
        ...(max !== undefined
            ? [MaxLength(max, { message: `${name} tối đa ${max} ký tự` })]
            : [])
    );
...
export const NumberRequired = (name: string, min?: number, max?: number) =>
    applyDecorators(
        ...
        ...(min !== undefined
            ...
            : []),
        ...(max !== undefined
            ? [Max(max, { message: `${name} phải nhỏ hơn hoặc bằng ${max}` })]
            : [])
    );
...
export const EnumRequired = (name: string, enumType: object) =>
    applyDecorators(
        ...
        IsEnum(enumType, {
            message: `${name} phải là: ${Object.values(enumType).join(', ')}`
        }),
        IsNotEmpty({ message: `${name} không được để trống` })
    );
