...
export class DashboardService {
  ...
  async getRevenueChart(
    ...
    try {
      ...
      transactions.forEach((tran) => {
        const date = tran.createdAt.toISOString().slice(0, 10);
        revenueByDate[date] =
          (revenueByDate[date] || 0) + Number(tran.totalPrice || 0);
      });
  ...
  async getTopStations(): Promise<any> {
    try {
      ...
      const stationCount: Record<number, { station: Station; count: number }> =
        {};
  ...
  async getTopAvgRatingStations(): Promise<any> {
    try {
      ...
      return {
        highest,
        lowest,
        message: 'Lấy trạm rating trung bình cao nhất và thấp nhất thành công',
      };
    } catch (error) {
      throw new InternalServerErrorException(
  ...
  async getUserMembershipStatsByMonthYear(
    ...
    try {
      ...
      allUsers.forEach((user) => {
        const activeMemberships = user.userMemberships
          ...
          .sort((a, b) => {
            ...
            const aTime = a.expiredDate ? new Date(a.expiredDate).getTime() : 0;
            const bTime = b.expiredDate ? new Date(b.expiredDate).getTime() : 0;
            return bTime - aTime;
          });
      ...
      const usersWithoutMembership = Object.values(userMainMembership).filter(
        (mId) => mId === null,
      ).length;
      ...
      const usersWithMembership = Object.values(userMainMembership).filter(
        (mId) => mId !== null,
      ).length;
