...
export class SlotService {
    ...
    async findAll(
        ...
        try {
            ...
            throw new InternalServerErrorException(
                error?.message || 'Lỗi hệ thống khi lấy danh sách slot'
            );
        }
    }
    ...
    async findById(id: number): Promise<any> {
        try {
            ...
            throw new InternalServerErrorException(
                error?.message || 'Lỗi hệ thống khi lấy chi tiết slot'
            );
        }
    }
                ...
                async (manager) => {
                    ...
                    if (existed) {
                        throw new BadRequestException(
                            'Slot đã tồn tại với battery này trong cabinet'
                        );
                    }
    ...
    async update(id: number, updateDto: UpdateSlotDto): Promise<any> {
        try {
            ...
            if (error instanceof NotFoundException) throw error;
            throw new InternalServerErrorException(
                error?.message || 'Lỗi hệ thống khi cập nhật slot'
            );
        }
    }

    async damagedSlot(id: number): Promise<any> {
        try {
            const result = await this.dataSource.transaction(
                async (manager) => {
                    const slot = await manager.findOne(Slot, { where: { id } });
                    if (!slot)
                        throw new NotFoundException('Slot không tồn tại');
                    if (slot.status === SlotStatus.DAMAGED)
                        throw new BadRequestException(
                            'Slot đã bị đánh dấu là hỏng trước đó'
                        );
                    if (slot.batteryId !== null)
                        throw new BadRequestException(
                            'Slot vẫn còn chứa pin, không thể chuyển sang trạng thái DAMAGED'
                        );
                    slot.status = SlotStatus.DAMAGED;
                    await manager.save(Slot, slot);
                    return { message: 'Đánh dấu slot bị hỏng thành công' };
                }
            );
            return result;
        } catch (error) {
            if (
                ...
                throw error;
            throw new InternalServerErrorException(
                error?.message || 'Lỗi hệ thống khi đánh dấu slot bị hỏng'
            );
        }
    }
        ...
        try {
            const result = await this.dataSource.transaction(
                async (manager) => {
                    ...
                    if (slot.status !== SlotStatus.DAMAGED)
                        throw new BadRequestException(
                            'Slot đã được kích hoạt sẵn'
                        );
                    if (slot.batteryId !== null)
                        throw new BadRequestException(
                            'Slot vẫn còn chứa pin, không thể chuyển về trạng thái EMPTY'
                        );
            ...
            if (
                ...
                throw error;
            throw new InternalServerErrorException(
                error?.message || 'Lỗi hệ thống khi khôi phục slot'
            );
        }
    }
}
